  ...
  return (
    <>
          ...
          <tbody>
            ...
            {!loading && flashcards.map((f) => (
              <tr key={f.id}>
                ...
                <td
                  ...
                  style={{
                    maxWidth: 200,
                    fontStyle: f.exemplo ? 'italic' : 'normal',
                    color: f.exemplo ? 'var(--fg-soft)' : 'var(--fg-faint)',
                  }}
                >
                ...
                <td>
                  <div className="cell-actions">
                    <button className="btn-icon" title="Editar" onClick={() => setEditing(f)}>
                      <Icon.edit />
                    </button>
                    <button className="btn-icon danger" title="Excluir" onClick={() => setDeleting(f)}>
                      <Icon.trash />
                    </button>
                  </div>
                </td>
            ...
            {!loading && flashcards.length === 0 && (
              <tr>
                <td colSpan={6}>
                  <div className="empty">
                    <h4>Nenhum flashcard cadastrado</h4>
                    <p>Crie um novo flashcard ou importe via XLSX.</p>
                  </div>
                </td>
              </tr>
            )}
      ...
      <ConfirmDelete
        ...
        message={deleting ? (
          <>Tem certeza que deseja excluir o <strong>Flashcard #{deleting.id}</strong>? Essa ação não pode ser desfeita.</>
        ) : ''}
      />
    </>
  );
